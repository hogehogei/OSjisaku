...
void putfont8( Sheet* sht, int x0, int y0, uint8_t color, uint8_t c )
{
    int x, y;
    int bxsize = sht->bxsize;
    int bysize = sht->bysize;
    ...
    uint8_t t, mask;
    const uint8_t* bitmap = get_fontbitmap( c );
...
Point putstr8( Sheet* sht, int x0, int y0, uint8_t color, const uint8_t* s )
{
    Point p;
    ...
    p.x = x; p.y = y;
    sheet_refreshsub( sht->ctl, x0, y0, max_x, max_y + 16 );
